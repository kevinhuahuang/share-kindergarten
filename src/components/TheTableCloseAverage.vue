  ...
  methods: {
    ...
    initTableColorIndex () {
      ...
      this.levelData.forEach((ary, i) => {
        ary.forEach((value, j) => {
          if (i < 2) {
            this.levelData[i][j] = Math.floor((1.15 + i * 0.1 - j * 0.05) * 100) / 100
          } else {
            this.levelData[i][j] = Math.floor((1.55 + i * 0.1 - j * 0.09) * 100) / 100
          }
        })
      })
    ...
    getTableData (host, param) {
      this.axios.get('/closeAverage/' + param).then(function (res) {
        host.tableData = []
