...
function writeChangeRate (data) {
  sqlSentence = 'DROP TABLE IF EXISTS close_average'
  ...
  sqlSentence = 'CREATE TABLE IF NOT EXISTS close_average' +
  ...
  connection.query(sqlSentence, function (err) {
    if (err) {
      ...
      console.log('创建数据库成功')
      sqlSentence = 'INSERT INTO close_average' +
  ...
  connection163.query(sqlSentence, function (err, result) {
    if (err) {
      ...
      indexAry.forEach(value => {
        temp = getAverageFromAry(aryTemp.slice(0, value))
        aryMin.push(temp)
        aryRate.push(Math.floor(aryTemp[0] / temp * 100) / 100)
      })
      temp = getAverageFromAry(aryTemp)
      aryMin.push(temp)
      aryRate.push(Math.floor(aryTemp[0] / temp * 100) / 100)
      // console.log(aryRes)
