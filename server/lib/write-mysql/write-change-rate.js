...
function writeChangeRate (data) {
  sqlSentence = 'DROP TABLE IF EXISTS change_rate'
  ...
  sqlSentence = 'CREATE TABLE IF NOT EXISTS change_rate' +
...
function getChangeRateDays (shareAry, callback) {
  ...
  sqlSentence = 'SELECT change_rate FROM _' + shareCode + ' ORDER BY date DESC LIMIT ' + 60
  ...
  connection163.query(sqlSentence, function (err, result) {
    if (err) {
      ...
      for (let i = 0; i < result.length; i++) {
        sumData = sumData + result[i]['change_rate']
        // console.log(sumData)
